...
void selection_sort_2arr(int* source, int* dest, bool* valid) {
    ...
    for (int i = 0; i < LIMIT; i++) {
        ...
        for (int j = 0; j < LIMIT; j++) {
            if (valid[j]) {
                if (smallest == -1 || source[j] < source[smallest]) {
                    smallest = j;
                }
            }
        }
...
int main() {
    ...
    // initialize valid array - at beginning the full array is valid
    for (int i = 0; i < LIMIT; i++) {
        valid[i] = true;
    }
    ...
    // initialize source array with random numbers from 0..RAND_RANGE again for fresh sorting
    generate_num(source);
