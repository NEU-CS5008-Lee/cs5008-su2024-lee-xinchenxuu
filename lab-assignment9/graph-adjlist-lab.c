...
void addConnection(Graph* graph, char* person, char* friend)
{
    ...
    // If the person is not in the graph, add them
    if (p == -1) {
        p = n;
        graph->adjLists[p] = createNode(person);
    }

    // If the friend is not in the graph, add them
    if (f == -1) {
        f = n;
        graph->adjLists[f] = createNode(friend);
    }

    // Add the friend to the person's list
    node* newNodeFriend = createNode(friend);
    newNodeFriend->next = graph->adjLists[p]->next;
    graph->adjLists[p]->next = newNodeFriend;

    // Add the person to the friend's list (because it's undirected)
    node* newNodePerson = createNode(person);
    newNodePerson->next = graph->adjLists[f]->next;
    graph->adjLists[f]->next = newNodePerson;
}
...
int main()
{
    ...
    /* Initializing adjacency matrix with values NULL */
